...
# Read all CSV files
data_sp = pd.read_csv('data/grid/sp.csv')
data_ka = pd.read_csv('data/grid/ka.csv')
data_kx0 = pd.read_csv('data/grid/kx0.csv')
data_kx1 = pd.read_csv('data/grid/kx1.csv')
data_k10 = pd.read_csv('data/grid/k10.csv')
...
def plot_with_confidence(ax, xi, probs, ci, label, linestyle='-'):
    ...
    ax.fill_between(xi, [c[0] for c in ci], [c[1] for c in ci], alpha=0.3)
...
ax.set_title('Regular Topology')
ax.legend()
ax.grid(True)
ax.set_xlim(0.5, 1)
ax.set_ylim(bottom=0, top=1)
plt.show()
plt.savefig('doc/bp_grid_conf.pdf')
