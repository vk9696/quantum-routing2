...
def find_assigned_paths(sd_pairs, n_sd, HQ_percent, HQ_seed):
    ...
    for pair in sd_pairs:
        ...
        try:
            ...
            valid_paths = [pf for pf in path_fidelities if pf[0][0] != 'NA']
            
            if valid_paths:
                selected_path = min(valid_paths, key=lambda x: x[0][0])[1]
                assigned_paths.append(selected_path)
...
def compute_fidelity_for_all_paths(sd_pairs_combinations, noise_rates, n_sd, HQ_percent, HQ_seed):
    ...
    for sd_pairs in sd_pairs_combinations:
        shortest_paths, path_order = find_assigned_paths(sd_pairs, n_sd, HQ_percent, HQ_seed)
...
def save_to_csv(all_paths_fidelity, n_sd, HQ_seed, HQ_percent):
    with open('data/wax/beta_0.275/k10_nsd_5_8_10_12.csv', 'a', newline='') as file:
...
def main():
    ...
    with open('data/wax/beta_0.275/k10_nsd_5_8_10_12.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["n_sd", "HQ_percent", "HQ_seed", "path", "sd_pairs", "path_length", "path_order", "fidelity"])

    for n_sd in {5, 8, 10, 12}:
