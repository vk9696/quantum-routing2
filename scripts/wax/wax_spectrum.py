...
def find_shortest_paths(sd_pairs, n_sd, eff_seed):
    ...
    for pair in sd_pairs:
        ...
        try:
            ...
            shortest_path = nx.dijkstra_path(network, source_node, destination_node)

            # Calculate the fidelity of the found path
            fidelity = calculate_fidelity(noise_rates, shortest_path)
            ...
            if fidelity != 'NA':
                ...
                network.remove_edges_from(used_edges)
...
def compute_fidelity_for_all_paths(sd_pairs_combinations, noise_rates, n_sd, eff_seed):
    ...
    for sd_pairs in sd_pairs_combinations:
        shortest_paths, path_order = find_shortest_paths(sd_pairs, n_sd, eff_seed)
        for order, path in zip(path_order, shortest_paths):  # Use zip to iterate through both path_order and shortest_paths simultaneously
            #logging.info(
                    ...
                    #)
            fidelity = calculate_fidelity(noise_rates, path)
...
def save_to_csv(all_paths_fidelity, n_sd, eff_seed):
    with open('data/wax_spectrum/sp_0.875.csv', 'a', newline='') as file:
...
def main():
    ...
    with open('data/wax_spectrum/sp_0.875.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["n_sd", "eff_seed", "path", "sd_pairs", "path_length", "path_order", "fidelity"])
